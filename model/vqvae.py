...
class VQAutoEncoder(BaseModel):
    ...
    def encode(self, x, x_a=None):
        # x = x.permute(0, 2, 1)  # 将 [bs, channels, seq_len] 转换为 [bs, seq_len, channels]
    ...
    def decode(self, quant):
        ...
        # dec = dec.permute(0, 2, 1)  # 将输出转换回 [bs, channels, seq_len]
        return dec
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.encoder_pos_embedding = PositionalEncoding(self.args.hidden_size)
        self.encoder_linear_embedding = LinearEmbedding(self.args.hidden_size, self.args.hidden_size)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, args, out_dim, is_audio=False):
        ...
        if args.quant_factor == 0:
            ...
            self.expander.append(nn.Sequential(
                nn.ConvTranspose1d(size, dim, 5, stride=2, padding=2, output_padding=1, padding_mode='zeros'),
                nn.LeakyReLU(self.args.neg, True),
                nn.InstanceNorm1d(dim, affine=args.INaffine)
            ))
            ...
            for _ in range(1, num_layers):
                self.expander.append(nn.Sequential(
                    nn.Conv1d(dim, dim, 5, stride=1, padding=2, padding_mode='zeros'),
                    nn.LeakyReLU(self.args.neg, True),
                    nn.InstanceNorm1d(dim, affine=args.INaffine),
                ))
        ...
        self.decoder_pos_embedding = PositionalEncoding(self.args.hidden_size)
        self.decoder_linear_embedding = LinearEmbedding(self.args.hidden_size, self.args.hidden_size)
