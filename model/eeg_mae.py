import sys
sys.path.append('/home/pengy/LongStorage/brain2024/')
# print(sys.path)
...
class MAEforEEG(nn.Module):
    ...
    """
    def __init__(self, time_len=1536, patch_size=8, embed_dim=1024, in_chans=64,
    ...
    def forward(self, imgs, img_features=None, valid_idx=None, mask_ratio=0.8):
...
class eeg_encoder(nn.Module):
    def __init__(self, time_len=1536, patch_size=8, embed_dim=1024, in_chans=64,
        ...
        self.blocks = nn.ModuleList([
            ...
            for i in range(depth)])
        self.norm = norm_layer(embed_dim)
        self.cls_layer = classify_network()
...
class classify_network(nn.Module):
    def __init__(self):
        super().__init__()
        self.maxpool = nn.Conv1d(192, 1, 1, stride=1)#nn.AdaptiveAvgPool1d((1))
