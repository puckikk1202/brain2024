...
wandb.init(project="eeg_clip", entity="ohicarip")
...
# Load the dataset
eeg_dataset = EEGDataset(eeg_dir, anno_dir, is_classification=False, is_clip=True, is_all=False ,is_staring=True, is_imagination=False)
split_ratio = 0.8
...
# Define the model
model = ViT(patch_size=16, emb_dim=1024, num_classes=8).to(device)
...
for epoch in range(num_epochs):
    ...
    correct = 0
    total = 0
    for i, (eeg_sample, img_feat) in enumerate(train_loader):
        eeg_sample, img_feat = eeg_sample.to(device, dtype=torch.float32), img_feat.to(device, dtype=torch.float32)
        optimizer.zero_grad()
    ...
    with torch.no_grad():
        for i, (eeg_sample, img_feat) in enumerate(test_loader):
            eeg_sample, img_feat = eeg_sample.to(device, dtype=torch.float32), img_feat.to(device, dtype=torch.float32)
            # output = model(eeg_sample)
            # loss = criterion(output, label)
            eeg_sample = eeg_sample.permute(0, 2, 1)
